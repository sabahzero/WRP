...
def get_edge_statement_text(s_qname, p_id, p_x, o_qname, qual=None, rev_extend=False):

    # Many different types of prdicates, so choose the right one.
    if not rev_extend and p_x:
        p_text = '{p_x}/{{prefix}}:{p}'.format(p=p_id, p_x=p_x)
    elif p_x:
        p_text = '{{prefix}}:{p}/^{p_x}'.format(p=p_id, p_x=p_x)
    else:
        p_text = '{{prefix}}:{p}'.format(p=p_id)

    # Simple one line if no qualifiers
    if not qual:
        out_text = "{s} {p} {o}".format(s=s_qname, p=p_text.format(prefix='wdt'), o=o_qname)
    # More complicated build if qualifiers involved
    else:
        out_text = '{s} {p1} [{p2} {o};'.format(s=s_qname, p1=p_text.format(prefix='p'),
                                                p2=p_text.format(prefix='ps'), o=o_qname)
        idx = out_text.index('[')
        shift = len(out_text[:idx+1])
        out_text += '\n' + ' '*shift + 'pq:{qual} ?qualifier]'.format(qual=qual)
    return out_text
...
def build_single_edge(s_qname, o_qname, sp_x, op_x, id, qual=None, rev_id=None, rev_qual=None, abbrev=None):
    ...
    # Currently edges with reverses only accept extensions on the object if both have them...
    if rev_id is not None:
        px = op_x if op_x else sp_x
        rev_ext = True if op_x else False

        edge_text = "\n\n    {{ {fwd} }}".format(fwd=get_edge_statement_text(s_qname, id, px, o_qname,
                                                                             qual, rev_ext))
        edge_text += "\n    UNION {{ {rev} }}".format(rev=get_edge_statement_text(o_qname, rev_id, px, s_qname,
                                                                                  rev_qual, not rev_ext))
    else:
